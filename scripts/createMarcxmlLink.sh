...
# This script links MARCXML files in global directory
...
# no action if link already exists
if [ -e "$MARC_XML_LINK" ]
then
    exit 0
fi
...
# create link
ln -s "$MARC_XML_FILE" "$MARC_XML_LINK"

if [ $? -ne 0 ]
then
	echo "ERROR: Could not create link to Marc XML file $MARC_XML_LINK -> $MARC_XML_FILE !"
	exit 1
fi
